...
RegisterNetEvent('esx_doorlock:doorBroken')
AddEventHandler('esx_doorlock:doorBroken', function()
	broken = true
	Citizen.Wait(120000)
	broken = false
end)
	...
	while true do
		...
		for k,v in ipairs(Config.DoorList) do
			...
			if v.distanceToPlayer and v.distanceToPlayer < v.maxDistance then
				...
				if (v.isAuthorized or GetSelectedPedWeapon(GetPlayerPed(-1)) == GetHashKey(Config.EnforcerModel)) then displayText = _U('press_button', displayText) end

				DrawText3Ds(v.textCoords.x, v.textCoords.y, v.textCoords.z, displayText)
				...
				if IsControlJustReleased(0, 38) then
					if v.isAuthorized then
						if not broken then
							...
							ESX.ShowNotification('Door lock is currently broken')
						end
					end
				end

				if IsPlayerFreeAiming(PlayerId()) then
					if (v.locked and IsPedShooting(GetPlayerPed(-1))) then
						...
						if GetEntityModel(door) == v.objHash then
							...
							if chance == 1 then
								v.locked = not v.locked
								TriggerServerEvent('esx_doorlock:updateState', k, v.locked) -- broadcast new state of the door to everyone
								TriggerEvent('esx_doorlock:doorBroken')
							end
						end
					end
				end
